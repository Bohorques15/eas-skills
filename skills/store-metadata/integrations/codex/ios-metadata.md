...
If none of the paths exists, stop and tell the user to install the skill first: `npx skills add Bohorques15/eas-skills --skill store-metadata` (see the pack README for manual-copy locations). In Codex the skill can also be invoked natively as `$store-metadata ios` once installed.
